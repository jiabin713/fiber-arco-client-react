...
const MutationDrawer = (props: { visible: boolean; onCancel: () => void; formRecord: Partial<PositionRecord> }) => {
  ...
  const afterOpen = () => {
    ...
    props.formRecord.id ? setTitle(`编辑 ${props.formRecord.name}`) : setTitle('新增职位');
  };
  const afterClose = () => {
    form.resetFields();
  };
